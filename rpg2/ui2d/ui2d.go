...
func NewUI(inputChan chan *game.Input, levelChan chan *game.Level) *ui {
	...
	ui.eventBackground = ui.GetSinglePixelTex(sdl.Color{0, 0, 0, 200})
...
func (ui *ui) loadTextureIndex() {
	...
	for scanner.Scan() {
		...
		tileRune := rune(line[0]) // not supporting unicode
		xy := line[1:]
...
func (ui *ui) Draw(level *game.Level) {
	...
	//level.Events is a circular array, render it so it scrolls up
	count := 0
	i := level.EventPos
