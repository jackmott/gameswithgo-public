...
// - Fix event scrolling
// - Make font a % of screen size
// - add spacing and such to event display
// - Make events more descriptive
// - Make monsters route around other monsters
// - Remove unecessary interface for attacking
// - Make Tile a struct
// - Bresenham
...
func (level *Level) AddEvent(event string) {
	fmt.Println("adding event at:", level.EventPos)
...
func bresenham(start Pos, end Pos) []Pos {
	//precompute how big to make this, to reduce GC pressure
	result := make([]Pos, 0)
...
func canWalk(level *Level, pos Pos) bool {
	if inRange(level, pos) {
		...
		//If there is a monster in the way say no
